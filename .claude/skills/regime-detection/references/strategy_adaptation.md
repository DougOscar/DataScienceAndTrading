...
| **Low vol + trend down** | Trend following (short), hedging | Buying dips, grid trading | Often precedes a sharper move if it accelerates. |
...
2. **Correlation spike**: If a broad risk-off/USD shock hits (e.g. a sharp DXY-proxy move or a major macro surprise), reset correlated FX/metals regimes to "danger" — everything tends to correlate in a shock, and pairs sharing a currency or risk-sentiment factor move together
3. **Volume death**: If (tick) volume drops below the 20th percentile of its own history, treat any trend signal as unreliable — thin-session moves are noisier
4. **Spread blowout**: If the MT5 `spread` column exceeds several multiples of its own rolling average (illiquid hours, news events), the regime is "illiquid" — reduce size and widen stops regardless of the volatility/trend classification
