...
- **Window**: match the trading timeframe — shorter for B3 intraday (20–50 bars), longer for D1 swing (50–100+ bars); crypto CFDs, where held, typically warrant shorter windows than FX/metals at the same bar interval
- **Percentile rank**: Rank current σ against the last 100 values → 0–100 score
- **Annualization**: use `periods_per_year` matching the bar interval and market (see `volatility-modeling`) if needed, but percentile rank doesn't require it
...
NATR of 0.3% means average true range is 0.3% of price. Useful for comparing across instruments — e.g. a quiet FX major vs. a metal vs. a B3 index future — where raw ATR in price units isn't comparable. Measure actual NATR levels on your own instruments rather than assuming a range.
...
Disadvantage:
- Does not capture gap risk (opening jumps) — relevant for the FX/metals
  weekend gap and B3's session-close-to-next-open gap; less of an issue
  for continuously-trading crypto CFDs
...
Most efficient estimator using OHLC data. Recommended whenever you have reliable OHLC — true for MT5 FX/metals bars and B3 futures data.
...
`smooth(x, N)` here is **Wilder smoothing** — an EMA with `alpha = 1/N`
(equivalently `ADX_t = ADX_{t-1} + (DX_t - ADX_t-1)/N`) — not the faster
standard EMA implied by `span=N` (`alpha = 2/(N+1)`). Using `span=N` instead
of Wilder smoothing produces a more reactive indicator whose values don't
match the conventional 20/25/40 thresholds below.
...
- Measure the actual H range on your own instrument/timeframe rather than
  assuming one — FX majors, metals, and crypto CFDs can all sit at
  different typical ranges, and it drifts with the regime
...
- Faster-moving instruments/timeframes (B3 intraday, crypto CFDs) generally
  warrant the faster (lower-h) setting; D1 swing FX/metals the more
  conservative one — confirm against your own false-alarm rate
...
3. Volume ratio: volume / rolling_mean_volume (captures participation — for
   MT5 this is tick volume, a noisier proxy than true traded volume)
...
- **Overfitting with >3 states**: stick to 2–3 states unless you have strong evidence more are warranted
