...
Computes 25+ features across price, volume, technical, spread, and time
categories. Tests each feature for stationarity using the ADF test and
generates a quality report including target correlation.
...
To use with real data, load your own OHLC(+spread) DataFrame (e.g. from an
MT5 export via quantlab's data loaders) in place of generate_synthetic_ohlcv().
MT5 bars are Bid prices and include a `spread` (points) column — a
point-in-time, causal feature; see compute_spread_features().
"""
...
def generate_synthetic_ohlcv(n_bars: int = 200, seed: int = 42) -> pd.DataFrame:
    ...
    # Generate a spread (points) column, as MT5 bars provide: mostly tight,
    # occasionally widening (e.g. low-liquidity hours, news events).
    base_spread = rng.normal(12.0, 2.0, n_bars).clip(min=3.0)
    spread_widening = rng.choice([1.0, 1.0, 1.0, 1.0, 3.0], size=n_bars)
    spread = base_spread * spread_widening
...
def compute_spread_features(df: pd.DataFrame) -> pd.DataFrame:
    """Compute spread-derived features.

    MT5 OHLC bars are Bid prices and include a `spread` (points) column —
    a point-in-time, causal feature (no lookahead: it reflects conditions
    at bar close, same as OHLCV) capturing execution cost and liquidity.
    If `spread` is not present (e.g. non-MT5 source), this is a no-op.

    Args:
        df: DataFrame with an optional 'spread' column (points).

    Returns:
        DataFrame with spread features added, if 'spread' was present.
    """
    if "spread" not in df.columns:
        return df
    s = df["spread"]
    df["spread_points"] = s
    df["spread_ratio"] = s / s.rolling(VOLUME_WINDOW).mean()
    return df
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Number of synthetic bars to generate (default: 200).",
    )
    parser.add_argument(
        "--output",
        type=str,
...
def main() -> None:
    ...
    # Get data — swap generate_synthetic_ohlcv() for your own MT5/quantlab
    # OHLC(+spread) loader to run this on real data.
