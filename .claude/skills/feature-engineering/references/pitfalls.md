...
- **Suspiciously high accuracy**: >70% accuracy on a short-horizon FX/metals or
  B3 classification task is almost certainly leakage. Real edge is typically 52-58%.
...
## 3. Survivorship / Contract-Roll Bias

Only analyzing instruments or contracts that are currently tradeable while
ignoring the ones that were delisted, discontinued, or expired.

### The Problem for FX / Metals / B3

FX majors and metals rarely disappear, so this bias is milder there — but it
still bites in two common ways:

- **B3 futures contract rolls**: WIN/WDO contracts expire and roll on a
  schedule. Naively concatenating raw contract series (instead of using a
  proper continuous/back-adjusted series) creates artificial jumps at roll
  dates, and dropping the illiquid tail of an expiring contract silently
  removes exactly the low-liquidity conditions a live strategy would face.
- **Broker/instrument list changes**: if the instrument universe used for
  training is "whatever the broker currently lists," discontinued or
  renamed symbols (exotic crosses, delisted CFDs) drop out of history
  retroactively, biasing training data toward instruments that persisted.

### How to Fix

- Use continuous/back-adjusted series for B3 futures, built consistently
  (e.g. roll on volume/open-interest crossover), not a naive concatenation.
- Keep the illiquid tail of expiring contracts in the data, or explicitly
  flag and account for it rather than silently dropping it.
- When building a multi-instrument universe, snapshot what was actually
  tradeable at each point in time rather than back-filling today's list.
...
- [ ] B3 contract rolls use a proper continuous series, not a naive concatenation
