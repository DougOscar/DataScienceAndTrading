...
Typical mean-reversion strategies: win rate 60-70%, payoff ratio 0.8-1.2, Kelly fraction 0.15-0.35. Use half-Kelly in practice. See the `position-sizing` skill for Kelly sizing details and MT5 lot-size constraints.
...
Use `scripts/pairs_scanner.py`, or this quick test:
...
cost_per_trade = 0.0005  # e.g. spread (points x point value) + commission, as a fraction
round_trip_cost = 2 * cost_per_trade  # cost of entry + exit
min_expected_move = round_trip_cost * 2  # need at least 2x round-trip cost as expected reversion
```
...
- **Contract-roll artifacts (B3 futures)**: Backtesting across expiries without a proper continuous/back-adjusted series creates phantom spread moves at roll dates
...
- **Not testing for regime changes**: A strategy that worked in a ranging period may fail once a directional macro regime takes hold
