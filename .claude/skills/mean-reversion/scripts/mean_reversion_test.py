...
To use with real data, load your own price/spread series (e.g. from an MT5
export via quantlab's data loaders) and pass it to print_report() in place
of generate_demo_data().
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"Number of synthetic bars to generate (default: {DEFAULT_LOOKBACK_BARS}).",
    )
    parser.add_argument(
        "--entry-z",
        type=float,
    ...
    print(f"Parameters: theta=0.1, mu=100.0, sigma=2.0, n={args.bars}")
    print("Swap generate_demo_data() for your own MT5/quantlab price or spread series for real data.\n")
    prices = generate_demo_data(n=args.bars, theta=0.1, mu=100.0, sigma=2.0)
