...
def generate_demo_assets(
    ...
    """Generate synthetic FX/metals price series with known cointegration structure.

    Creates:
    - Asset A, B: cointegrated pair (strong mean-reverting spread) — e.g. two
      USD-crosses sharing a common USD factor
    - Asset C: partially correlated with A (weaker cointegration) — e.g. a metal
    ...
    Returns:
        Dict mapping instrument name to price array.
    """
    rng = np.random.default_rng(seed)
    names = ["EURUSD", "GBPUSD", "XAUUSD", "USDJPY", "AUDUSD"][:n_assets]
    assets: dict[str, np.ndarray] = {}

    # Common factor (broad USD direction)
    market = np.cumsum(rng.normal(0.001, 0.02, n_bars))

    # Asset A (EURUSD): USD factor + own noise
    eur_noise = np.cumsum(rng.normal(0, 0.01, n_bars))
    assets["EURUSD"] = 1.08 * np.exp(0.5 * market + 0.5 * eur_noise)

    # Asset B (GBPUSD): cointegrated with EURUSD (shared USD exposure + mean-reverting spread)
    spread_noise = np.zeros(n_bars)
    spread_noise[0] = rng.normal(0, 0.01)
    for i in range(1, n_bars):
        spread_noise[i] = 0.9 * spread_noise[i - 1] + rng.normal(0, 0.01)
    assets["GBPUSD"] = 1.27 * np.exp(0.5 * market + 0.5 * eur_noise + spread_noise)

    # Asset C (XAUUSD, gold): partially correlated with the USD factor
    gold_noise = np.cumsum(rng.normal(0, 0.015, n_bars))
    assets["XAUUSD"] = 2000 * np.exp(0.3 * market + 0.7 * gold_noise)

    # Asset D (USDJPY): trending / momentum
    drift = np.cumsum(rng.normal(0.002, 0.03, n_bars))
    assets["USDJPY"] = 150 * np.exp(drift)

    # Asset E (AUDUSD): independent random walk
    aud_walk = np.cumsum(rng.normal(0, 0.02, n_bars))
    assets["AUDUSD"] = 0.65 * np.exp(aud_walk)
...
def main() -> None:
    ...
    if not args.demo:
        ...
        print("\nTo extend with real data, load your own price series (e.g. from an")
        print("MT5 export via quantlab's data loaders) in place of generate_demo_assets().")
        sys.exit(1)

    print("Running in DEMO mode with synthetic data.")
    print(f"Generating {NUM_DEMO_ASSETS} assets with known cointegration structure.\n")
    print("  EURUSD, GBPUSD: Cointegrated pair (shared USD factor + MR spread)")
    print("  XAUUSD: Partially correlated with EURUSD")
    print("  USDJPY: Trending (momentum, not mean-reverting)")
    print("  AUDUSD: Independent random walk\n")
