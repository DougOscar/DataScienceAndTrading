...
Environment Variables:
    ACCOUNT_SIZE: Total account value in USD (default: 15000)

Note: this demo simplifies `position_value = units * price` for every
instrument. Real FX/metals/B3 position value and risk depend on contract
size / point (tick) value per the symbol spec (see the "MT5 Lot Constraints"
section of SKILL.md) — plug those conversions in for real positions.
"""
...
def get_demo_positions() -> list:
    ...
    return [
        ...
        Position(
            symbol="WIN",
            entry_price=126500.0,
            current_price=128200.0,
            stop_loss=124800.0,
            units=0.02,
            sector="b3_index",
        ),
