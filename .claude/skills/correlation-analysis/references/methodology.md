...
### When Pearson Fails Under Heavy Tails

Return distributions often violate normality — most sharply for crypto CFDs,
but also FX/metals around news/events (kurtosis well above 3, the normal
value). Heavy tails mean:
...
- Handles heavy tails naturally

### When to Use

- Good default when return distributions are heavy-tailed (news/event risk, crypto CFDs)
- When comparing instruments with different volatility profiles
...
4. **Domain knowledge**: group by known structure and verify against the clustering — e.g. USD-crosses, JPY/CHF safe havens, commodity/risk currencies (AUD, NZD, CAD), metals, and (if held) crypto CFDs
...
In risk assets generally (equities, commodity/risk currencies, crypto CFDs), λ_L >> λ_U is common — instruments crash together more than they rally together. Measure both on your own instruments rather than assuming this holds everywhere (it need not for, e.g., a safe-haven pair like USDJPY vs. a risk currency).
...
4. **Survivorship / contract-roll bias**: only analyzing instruments/contracts that are still tradeable, or naively concatenating B3 futures across contract rolls, distorts correlation estimates
5. **Frequency mismatch**: daily correlation ≠ hourly correlation (Epps effect: higher frequency → lower correlation)
6. **Confusing correlation with causation**: two instruments both correlated with a common factor (e.g. broad USD strength) will appear correlated with each other
