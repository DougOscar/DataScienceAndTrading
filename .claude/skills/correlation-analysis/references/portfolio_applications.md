...
|-----------|----------|-------|------------|
| All USD-crosses (EURUSD, GBPUSD, AUDUSD, ...) | 10 | ~2-3 | Poor — dominated by a shared broad-USD factor |
| Mixed FX + metals | 10 | ~4-5 | Moderate |
| FX + metals + B3 (different drivers) | 10 | ~6-7 | Good diversification |

Illustrative only — compute `N_eff` on your actual instrument set; it moves
with the prevailing correlation regime.
...
1. Compute correlation matrix for instrument universe
2. Filter pairs with correlation > 0.8
3. Verify economic rationale (shared currency/factor exposure, similar mechanics)
4. Test for cointegration (see the `mean-reversion` skill's pairs-trading section and `scripts/pairs_scanner.py`)
...
- **Correlation breakdown**: previously correlated instruments diverge permanently
- **Regime change**: new market regime invalidates historical relationship
- **Execution**: legs can have different liquidity/spread (especially one FX/metals leg vs. one B3 or crypto CFD leg); slippage on one leg erodes the spread capture
- **Always test cointegration**, not just correlation — see the `mean-reversion` skill
...
Compare crisis correlation vs full-sample correlation. If crisis correlation is materially higher (common across risk assets in a risk-off event, and often more pronounced for crypto CFDs), standard diversification metrics overstate the true benefit.
