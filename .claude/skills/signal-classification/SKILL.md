...
1. **Feature engineering** — compute technical indicators, spread features, volume features (see `feature-engineering` skill)
...
Typical finding: optimal thresholds often land well above 0.5 (e.g. 0.60-0.75),
but the actual value is instrument- and model-specific — don't assume a fixed range.

**Critical: select the threshold on a validation/inner fold, never on the
final OOS or holdout set used for reporting.** Optimizing `optimize_threshold`
directly against the test window you're about to report performance on is a
selection-on-the-test-set leak — the reported profit factor will be inflated
by the same overfitting `quant-trading-research` warns about for parameter
selection generally. Correct procedure: within each walk-forward training
window, hold out a validation slice (or use nested/purged CV), pick the
threshold there, then apply that fixed threshold to the untouched OOS test
window for the performance you actually report.

## Practical Considerations

### Training Window Length

How fast the tradable relationship drifts (broker/spread conditions, macro
regime) sets the training window, not a fixed rule — FX/metals swing models
often tolerate longer windows than a fast-moving intraday B3 model, but
always confirm with walk-forward performance rather than assuming either.
...
A model with 55% accuracy sounds good, but after realistic round-trip costs
it may be unprofitable. Use the actual MT5 `spread` column (points × point
value) plus commission and slippage for FX/metals, or exchange fees plus
slippage for B3 futures — not a flat assumption. Always evaluate signals net
of costs:

```python
net_return = gross_return - round_trip_cost  # measured on your own instrument
```
...
2. **Optimize threshold for profit factor**, not accuracy — a high-precision, low-recall model beats a high-accuracy one — but select that threshold on a validation/inner fold, never on the final OOS/holdout you report
3. **Training window length follows regime-drift speed**, not a fixed rule — confirm with walk-forward performance rather than assuming
4. **Monitor feature decay** — retrain when rolling metrics drop below baseline
5. **Always evaluate net of costs** — a 55% accurate model may be unprofitable after realistic spread/commission/slippage
