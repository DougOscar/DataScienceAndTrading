...
7. Probability threshold optimization for profit factor (demo simplifies this
   by tuning on the reported OOS set -- see the WARNING in Step 4 below; in
   real use, tune on a validation/inner fold instead)
...
def main() -> None:
    ...
    #
    # WARNING (demo simplification): this optimizes the threshold on the same
    # concatenated OOS predictions that print_report() then evaluates and
    # reports. That is threshold selection on the reporting set, which will
    # inflate the reported profit factor — the same leak as selecting a
    # strategy's parameters on its own test set. In real use, select the
    # threshold on a validation/inner fold (e.g. held out within each
    # walk-forward training window) and apply the fixed, already-chosen
    # threshold to a separate, untouched OOS window for the numbers you
    # actually report. See SKILL.md's Threshold Optimization section.
    ...
    print("  NOTE: threshold optimized on the same OOS set being reported below")
    print("        (demo simplification) -- in real use, fit the threshold on a")
    print("        validation/inner fold, not the final holdout being reported.")
