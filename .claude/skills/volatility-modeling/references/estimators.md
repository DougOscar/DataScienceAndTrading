...
| N | Annualization factor (periods_per_year): ~260 for FX/metals daily bars (5-day week; 252 also common), 365 for crypto CFDs (7-day week), bars_per_day × trading_days_per_year for intraday |
...
σ_annual = 0.03324 × sqrt(260) = 0.536 = 53.6%   # FX/metals daily (N=260)
```
...
σ_annual = 0.03622 × sqrt(260) = 0.584 = 58.4%   # FX/metals daily (N=260)
```
...
σ_annual = 0.04571 × sqrt(260) = 0.737 = 73.7%   # FX/metals daily (N=260)
```
...
**Properties:**
- Handles opening/gap jumps correctly — relevant for FX/metals (weekend gap
  between Friday close and Sunday open) and B3 futures (gap between one
  session's close and the next session's open).
- Less useful for crypto CFDs, which trade continuously with no scheduled gap.
...
**Parameters:** Fit ω, α, β via MLE on your own instrument rather than
assuming a fixed range — typical parameter ranges differ materially between
a quiet FX major, metals, B3 futures, and crypto CFDs. As a sanity check,
α + β should land somewhere in the 0.85–0.99 range for most liquid
instruments when volatility clusters at all; values outside that suggest a
data or fitting problem.

**Properties:**
- Captures volatility clustering and mean reversion.
- Produces a vol term structure (unlike EWMA).
- Requires numerical optimization.
- Student-t innovations better capture fat tails — most pronounced in
  crypto CFDs, but present in FX/metals and B3 futures too, especially
  around news/event risk.
