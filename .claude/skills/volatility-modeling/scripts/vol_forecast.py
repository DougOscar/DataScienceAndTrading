...
Usage:
    ...
    python scripts/vol_forecast.py --periods-per-year 260   # FX/metals daily

Dependencies:
    uv pip install pandas numpy scipy

To use with real data, load your own return series (e.g. from an MT5
export via quantlab's data loaders) in place of generate_demo_returns().
"""
...
class EWMAModel:
    ...
    def __init__(
        self, lam: float = EWMA_LAMBDA, periods_per_year: float = ANNUALIZATION_FACTOR
    ) -> None:
        self.lam = lam
    ...
    def forecast(self, horizons: list[int]) -> dict[int, float]:
        ...
        for h in horizons:
            ...
            ann_vol = daily_vol * np.sqrt(self.periods_per_year)
            forecasts[h] = ann_vol
        return forecasts
...
class GARCHModel:
    ...
    def __init__(self, periods_per_year: float = ANNUALIZATION_FACTOR) -> None:
    ...
    def fit(self, returns: pd.Series) -> bool:
        ...
        # Initial guess: typical daily-bar parameters (alpha/beta ranges
        # differ by market — recalibrate for FX/metals vs. crypto CFDs)
        x0 = np.array([
    ...
    def long_run_vol_annual(self) -> float:
        ...
        return np.sqrt(self.long_run_variance) * np.sqrt(self.periods_per_year)
    ...
    def forecast(self, horizons: list[int]) -> dict[int, float]:
        ...
        for h in horizons:
            ...
            ann_vol = np.sqrt(forecast_var) * np.sqrt(self.periods_per_year)
            forecasts[h] = ann_vol
...
def print_model_params(ewma: EWMAModel, garch: GARCHModel) -> None:
    ...
    print(f"    Current vol:     {np.sqrt(ewma.current_var) * np.sqrt(ewma.periods_per_year) * 100:.1f}% ann.")
    ...
    print(f"    Current vol:     {np.sqrt(garch.current_var) * np.sqrt(garch.periods_per_year) * 100:.1f}% ann.")
...
def print_term_structure(garch: GARCHModel) -> None:
    ...
    curr_vol = np.sqrt(garch.current_var) * np.sqrt(garch.periods_per_year) * 100
...
def main() -> None:
    ...
    print("Swap generate_demo_returns() for your own MT5/quantlab return series for real data.\n")
    ...
    # Fit EWMA
    ewma = EWMAModel(lam=args.lambda_, periods_per_year=args.periods_per_year)
    ewma.fit(returns)

    # Fit GARCH
    garch = GARCHModel(periods_per_year=args.periods_per_year)
