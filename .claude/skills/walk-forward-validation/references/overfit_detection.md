...
- Use `periods_per_year` matching the bar interval and market hours: ~260 for FX/metals daily bars (5-day week; 252 also common), 365 for crypto CFDs (7-day week), or `bars_per_day * trading_days_per_year` for intraday (e.g. B3 sessions).
...
For a B3 intraday strategy with 1-minute-bar SR = 0.01:
- At 95% confidence: MinBTL ≈ 38,416 1-minute observations (roughly a few hundred sessions, depending on session length)
