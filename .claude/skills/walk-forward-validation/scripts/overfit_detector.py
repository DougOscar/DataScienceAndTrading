...
def deflated_sharpe_ratio(
    ...
    Args:
        ...
        annualization: Annualization factor, e.g. sqrt(periods_per_year)
            (~sqrt(260) for FX/metals daily bars, sqrt(365) for crypto CFDs).
...
def run_demo() -> None:
    ...
    print("-" * 72)
    annualization = np.sqrt(252)  # FX/metals daily bars (~260 also common)
    sharpes = []
    for i in range(n_strategies):
