...
def compute_sharpe(returns: np.ndarray, annualization: float = 252.0) -> float:
    ...
    Args:
        ...
        annualization: Periods per year. FX/metals daily bars ~260
            (5-day week; 252 also common); crypto CFDs 365 (7-day week);
            intraday = bars_per_day * trading_days_per_year.
...
def generate_synthetic_prices(
    ...
    """Generate synthetic daily price data with realistic FX/metals volatility.
    ...
    Args:
        ...
        periods_per_year: Bars per year used to de-annualize drift/vol
            (~260 for FX/metals daily bars, 365 for crypto CFDs).

    Returns:
        DataFrame with 'date', 'close', and 'returns' columns.
    """
    rng = np.random.default_rng(seed)
    daily_return = annual_return / periods_per_year
